...
import placeholderimg from "../../../images/placeholder-images/placeholder-cake.png";
...
const CakesMenuComponent = (props) => {
    const { isAuthenticated, loginWithRedirect } = useAuth0();
    const cakeItems = props.searchBase === 'menu' && props.type !== 'supplier' ? props.allKitchensMenu.cake.items : props.cake.items;
    const submitButton = (props.isMenuItemUpdate || props.isSetDailyMenuItem)
        ? <div className='form-check form-check-inline'>
            <button type="submit" className="continue-button background-color">
                <span className='spinner-border myInput text-primary spinner-border-sm'></span> Updating... </button>
        </div>
            ...
            <Scrollbars style={{ height: "60vh" }} className="menu-without-add-btn mobile-background" id='itemsDays'>
                ...
                {(props.isMenuItemFetching || props.isKitchensFetching || props.isMenuFetching)
                    ...
                    : <div className='row mx-0 mt-2 cardMargin'>
                        {cakeItems.length ? cakeItems.map((items, index) => {
                        ...
                        }) : props.cakeMenuKitchens.map((kitchen, index) => {
...
export default connect(mapStateToProps, null)(CakesMenuComponent);